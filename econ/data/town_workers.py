...
_RAW_WORKERS = [
    ...
    dict(key="quarryman", name="Quarryman", emoji="⛏️", linked="quarry",
         base_gold=300, base_qty=5, flavor="Swings a pick from sunup to sundown."),
    dict(key="blast_foreman", name="Blast Foreman", emoji="🧨", linked="quarry",
         base_gold=450, base_qty=6, flavor="Knows exactly how much powder is too much."),
    dict(key="woodcutter", name="Woodcutter", emoji="🪓", linked="sawmill",
         base_gold=300, base_qty=5, flavor="Never misses the same notch twice."),
    dict(key="sawyer", name="Sawyer", emoji="🪚", linked="sawmill",
         base_gold=450, base_qty=6, flavor="Turns rough trunks into true planks."),
    dict(key="brickmaker", name="Brickmaker", emoji="🧱", linked="brickworks",
         base_gold=550, base_qty=7, flavor="Packs the mould the same way every time."),
    dict(key="kiln_master", name="Kiln Master", emoji="🔥", linked="brickworks",
         base_gold=700, base_qty=8, flavor="Reads a kiln's heat like a book."),
    dict(key="ore_smelter", name="Ore Smelter", emoji="🔥", linked="foundry",
         base_gold=700, base_qty=8, flavor="The furnace never truly cools."),
    dict(key="blacksmiths_apprentice", name="Blacksmith's Apprentice", emoji="🔨", linked="foundry",
         base_gold=850, base_qty=8, flavor="Eager, and getting better by the week."),
    dict(key="herbalist", name="Herbalist", emoji="🌿", linked="herb_garden",
         base_gold=1_150, base_qty=9, flavor="Knows every plot by smell alone."),
    dict(key="greenhouse_keeper", name="Greenhouse Keeper", emoji="🪴", linked="herb_garden",
         base_gold=1_450, base_qty=10, flavor="Coaxes a second harvest out of tired soil."),
    dict(key="weaver", name="Weaver", emoji="🧵", linked="weavers_yard",
         base_gold=1_300, base_qty=9, flavor="The loom barely pauses for breath."),
    dict(key="dye_master", name="Dye Master", emoji="🎨", linked="weavers_yard",
         base_gold=1_550, base_qty=10, flavor="Colours that don't fade in the wash."),
    dict(key="mason", name="Mason", emoji="🪨", linked="masons_workshop",
         base_gold=1_850, base_qty=11, flavor="Squares every block before it's set."),
    dict(key="stonecarver", name="Stonecarver", emoji="🗿", linked="masons_workshop",
         base_gold=2_450, base_qty=12, flavor="Turns a plain block into a signature."),
    dict(key="gem_cutter", name="Gem Cutter", emoji="💎", linked="gem_cutters_den",
         base_gold=3_150, base_qty=14, flavor="One wrong tap ruins a week's work -- theirs never do."),
    dict(key="jewelers_apprentice", name="Jeweler's Apprentice", emoji="🔍", linked="gem_cutters_den",
         base_gold=3_850, base_qty=15, flavor="Learning the trade on the town's own stones."),

    # ── four town-wide hires, each boosting one bonus/utility building ──
    dict(key="town_crier", name="Town Crier", emoji="📯", linked="guild_hall",
         base_gold=2_150, base_qty=12, flavor="Talks up every deal before it's even struck."),
    dict(key="scribe", name="Scribe", emoji="✍️", linked="great_library",
         base_gold=2_550, base_qty=13, flavor="Copies down every lesson worth keeping."),
    dict(key="guard_captain", name="Guard Captain", emoji="🛡️", linked="watchtower",
         base_gold=4_300, base_qty=16, flavor="Nothing gets past the wall on their watch."),
    dict(key="steward", name="Steward", emoji="🗝️", linked="storehouse",
         base_gold=2_450, base_qty=12, flavor="Knows exactly what's in every crate and why."),
]
